...
ErrorOr<int> serenity_main(Main::Arguments arguments){
    ...
    parser.add_option(cpu_timing, "CPU timing in cycles per second. Default is 700 (standard CHIP-8 timing).", "cpu-timing", 'c', "cpu-timing");
    parser.add_positional_argument(rom_path, "Path to the Rom", "rom-path");
    parser.parse(arguments);

    auto c8 = Chip8(screen_size_factor, cpu_timing);
